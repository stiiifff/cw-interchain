...
pub fn save_reply_payload<T>(store: &mut dyn Storage, payload: &T) -> StdResult<u64>
where
    T: Serialize + ?Sized,
{
...
pub fn read_reply_payload<T: DeserializeOwned>(store: &dyn Storage, id: u64) -> StdResult<T> {
    let data = REPLY_QUEUE_ID.load(store, id)?;
    from_json(Binary(data))
}

pub fn save_sudo_payload<T>(
...
pub fn read_sudo_payload<T: DeserializeOwned>(
    store: &dyn Storage,
    channel_id: String,
    seq_id: u64,
) -> StdResult<T> {
